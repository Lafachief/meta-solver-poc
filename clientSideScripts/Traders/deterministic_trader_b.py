...
def main():
    try:
        logger.info("Starting Deterministic Trader B...")
        ...
        with open(config_path) as f:
            private_key = json.load(f)["privateKey_b"]
        ...
        executed_trades = execute_trade_sequence(client, price_tracker.current_price)
        logger.info("Trade sequence completed.")
        
        # Print trade summary
        logger.info("\n=== Trade Summary ===")
        for i, trade_id in enumerate(executed_trades, 1):
            logger.info(f"Trade {i}: Order ID {trade_id}")
        logger.info("===================")
